...
LOG_BLOCKS = {
    ...
    "motors": (20, ["motor.m1", "motor.m2", "motor.m3", "motor.m4", "motor.m1req", "motor.m2req", "motor.m3req", "motor.m4req"]),
    ...
    "system": (200, ["ctrltarget.thrust", "pm.vbat", "pm.batteryLevel", "pm.state", "pm.chargeCurrent", "radio.rssi", "radio.isConnected", "radio.numRxBc", "radio.numRxUc", "supervisor.info", "supervisor.accNorm"]),
    "environment": (200, ["baro.asl", "baro.pressure", "baro.temp", "mag.x", "mag.y", "mag.z"]),
}
BASE_FIELDS = [
    "time_s", "phase", "phase_id", "odom_received",
...
class CrazyflieHover(Node):
    def __init__(self):
        ...
        self.csv_path = f"crazyflie_hover_{datetime.now():%Y%m%d_%H%M%S}.csv"
    ...
    def hover_mission(self):
        ...
        self.run_vertical_motion(x0, y0, z0, self.hover_z, 2.5)
        self.set_phase("hover", 2)
        ...
        while not self.shutdown_flag.is_set() and time.perf_counter() - start < self.hover_duration:
            ...
            time.sleep(1.0 / self.command_rate)
        self.set_phase("landing", 3)
        self.get_logger().info("Landing...")
        self.run_vertical_motion(x0, y0, self.hover_z, z0, 2.5)
        self.set_phase("landed", 4)
        self.stop_motors()
    ...
    def write_csv(self):
        ...
        with self.lock:
            ...
            row.update({"time_s": time.perf_counter() - self.t0, "phase": self.phase, "phase_id": self.phase_id, "odom_received": int(self.odom_received)})
